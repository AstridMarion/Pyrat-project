...
class Backtracking (Player):
    ...
    def preprocessing ( self:       Self,
        ...
        route=self.route_mouse(self.Backtracking(graph,game_state.player_locations[self.name]),graph_retour)
    ...
    @override
    def Backtracking(self:Self,graph:Graph,source:Integral)->List[Integral]:
        """ 
            This method performs a backtracking algorithm on a graph thanks to the auxiliary 
            fonction boucle.
        ...
        """
        def boucle(path:list[Integral],weight:float,weight_best:float)->Tuple[float,List[Integral]]:
            """ 
            ...
            for voisin in graph.get_neighbors(path[-1]):
                if not voisin in path:
                    ...
                    weight_new=weight+graph.get_weight(path[-1],voisin)
                    #stop the search if it's not optimized
                    if weight_new >=weight_min :
                        continue
                    #search deeper if it is optimized
                    (weight_final,path_final)=boucle(path_new,weight_new,weight_min)
                    if weight_final<weight_min:
                        weight_min=weight_final
                        path_min=path_final
            if not drapeau:
                return (float('inf'),[])
            return (weight_min,path_min)
